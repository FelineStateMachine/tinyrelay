...
func (t *Tenant) llmsHTTP(w http.ResponseWriter, r *http.Request) {
	...
	lines = append(lines,
		...
		"- Signed write tools: publish_event, create_issue, create_pull_request, comment, set_status, post_message, start_thread, reply_in_thread, react, publish_wiki_page, propose_wiki_merge, publish_site, create_room, request_decision, request_grant, request_job, accept_job, job_progress, job_result, job_error and cancel_job. Builders return an unsigned event when called with plain fields: sign it with your Nostr key and call again with the signed event as event. This first call does not publish anything. publish_event accepts an already signed event.",
		"",
		...
		"- Long tasks: request_job publishes a kind 43001 request in a room with p tags for the keys asked. Only a key the request asked answers it: accept_job (43002), job_progress (43003) and then job_result (43004) or job_error (43006); the requester withdraws it with cancel_job (43005). Every answer names the request in e, the requester in p and keeps the request's h room tag. Chat folds progress, results and errors into task cards; list_jobs and read_job return the work visible to your key.",
		...
		"## Chat and Social",
		"",
		"- Chat: "+base+"/chat combines NIP-29 group rooms and NIP-17 one-to-one messages. Existing /rooms URLs and room MCP tool names remain supported. Group messages use kind 9; kind 11 threads and kind 12 replies remain supported. Kind 9 replies use NIP-10 root and reply markers. The web UI shows one level of replies under the original root; follow-ups stay in that thread. Unsigned reply_in_thread templates resolve reply IDs to the original root. Room attachments use NIP-92 metadata. The chat guide documents 0xchat, Flotilla and Buzz compatibility boundaries.",
		...
		"- Social: "+base+"/social combines kind 1 notes, kind 30023 articles and NIP-F4 kind 54 podcast episodes. Podcast shows use kind 10154 metadata. Note replies use NIP-10; article and podcast comments use NIP-22 kind 1111; reactions use NIP-25 kind 7, including NIP-30 emoji. Articles and episode notes render Markdown; notes and comments keep plain text with links and media embeds. Media uses NIP-92. Use publish_event for signed Social events; the Social guide covers formats and interoperability.",
		"- /articles redirects to Social. Mixed feeds are at "+base+"/social.json and "+base+"/social.xml; existing /articles.json and /feed.xml article feeds remain available. Native podcast RSS is at "+base+"/social/podcasts.rss?author=<show-pubkey>; the podcasts relay view refreshes when shows or episodes change.",
		"",
		"## Files",
		"",
