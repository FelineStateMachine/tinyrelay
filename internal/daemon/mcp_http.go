...
func (t *Tenant) llmsHTTP(w http.ResponseWriter, r *http.Request) {
	...
	lines = append(lines,
		...
		"## Chat and Social",
		"",
		"- Chat: "+base+"/chat combines NIP-29 group rooms and NIP-17 one-to-one messages. Existing /rooms URLs and room MCP tool names remain supported. Group messages use kind 9; kind 11 threads and kind 12 replies remain supported. Kind 9 replies use NIP-10 root and reply markers. Room attachments use NIP-92 metadata. The rooms guide documents 0xchat, Flotilla and Buzz compatibility boundaries.",
