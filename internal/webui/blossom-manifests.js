...
(() => {
  ...
  const writeValue = (w, v) => {
    ...
    if (typeof v === "number") {
      if (Number.isInteger(v)) return writeInteger(w, v);
  ...
  class MessagePackReader {
    constructor(b) {
      this.input = asBytes(b);
      this.offset = 0;
    }
    need(n) {
      if (this.offset + n > this.input.length) throw new Error("truncated MessagePack");
    }
    u8() {
      this.need(1);
    ...
    n(len) {
      this.need(len);
      const x = this.input.slice(this.offset, this.offset + len);
      this.offset += len;
      return x;
    }
  }
  ...
  const readValue = (r, depth = 0) => {
    ...
    if (t === 204) n = r.u8();
      ...
      n = d.getFloat64(0);
    } else if (t === 217) n = textDecoder.decode(r.n(r.u8()));
    else if (t === 218) n = textDecoder.decode(r.n((r.u8() << 8) | r.u8()));
  ...
  const validateMeta = (m, depth = 0) => {
    ...
    for (const [k, v] of Object.entries(m)) {
      ...
      if (
        ...
        throw Error("metadata is not JSON-compatible");
      if (v && typeof v === "object") {
        if (Array.isArray(v)) {
  ...
  const encodeManifest = node => {
    ...
    for (const x of node.l) {
      ...
      for (const k of fields) {
        writeValue(w, k);
        writeValue(w, k === "h" || k === "k" ? asBytes(x[k]) : x[k]);
      }
    }
    writeValue(w, "t");
    writeValue(w, node.t);
  ...
  const decodeManifest = (bytes, options = {}) => {
    ...
    const r = new MessagePackReader(b),
      node = readValue(r);
    if (r.offset !== b.length) throw Error("trailing MessagePack data");
    validateNode(node);
    if (node.t === 3)
      for (const x of node.l) {
