...
(() => {
  ...
  const fromHex = (value) => {
    if (typeof value !== "string" || !/^[0-9a-f]{64}$/.test(value))
      throw new Error("expected a 64-character lowercase hex key");
  ...
  const aesKey = async (key) => {
    const material = await crypto.subtle.importKey("raw", key, "HKDF", false, [
      "deriveKey",
    ]);
  ...
  const decryptCHK = async (ciphertext, key, hash) => {
    ...
    if (hex(await digest(input)) !== hash)
    ...
    if (!equal(await digest(plain), chkKey))
  ...
  const parseURI = (value) => {
    if (typeof value !== "string" || !value.startsWith("blossom:"))
      throw new Error("expected a Blossom URI");
    const parsed = new URL(
      value.slice("blossom:".length),
      "https://blossom.invalid",
    );
    const match = parsed.pathname.match(
      /^\/([0-9a-f]{64})(?:\.([A-Za-z0-9]+))?$/,
    );
    if (!match || parsed.searchParams.get("enc") !== "chk-v1")
      throw new Error("invalid CHK Blossom URI");
