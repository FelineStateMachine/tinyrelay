...
(function (root) {
  ...
  function relayList(event, pubkey) {
    if (Array.isArray(event)) {
      event = event
        .filter(
          (candidate) =>
            candidate?.kind === 10050 && candidate.pubkey === pubkey,
        )
    ...
    const signer = api();
    if (!signer.verifyEvent(event))
      throw new Error("invalid kind 10050 relay list signature");
    const relays = [
      ...new Set(
        event.tags
  ...
  function normalizePubkey(value) {
    ...
    if (typeof decoded === "string" && /^[0-9a-f]{64}$/i.test(decoded))
  ...
  function past(now) {
    return Math.max(
      0,
      Math.floor(now) - Math.floor(Math.random() * (2 * DAY + 1)),
    );
  }
  ...
  async function queryRelayList(pubkey, options) {
    if (!root.tiny?.signedFetch)
      throw new Error("signed local query is unavailable");
    const response = await root.tiny.signedFetch(
      "/query",
      "POST",
  ...
  function unsignedRumor(input, sender, now) {
    ...
    if (!/^[0-9a-f]{64}$/i.test(input.ciphertextHash || ""))
      throw new Error("ciphertextHash must be SHA-256 hex");
    if (!input.key || !input.nonce)
      throw new Error(
        "encrypted file messages require a decryption key and nonce",
      );
    ...
    if (input.plaintextHash) {
      if (!/^[0-9a-f]{64}$/i.test(input.plaintextHash))
        throw new Error("plaintextHash must be SHA-256 hex");
  ...
  async function encryptFor(signer, recipient, plaintext) {
    ...
    if (signer.nip44?.encrypt)
      return signer.nip44.encrypt(recipient, plaintext);
  ...
  async function seal(rumor, recipient, signer, now) {
    ...
    if (
      ...
      throw new Error("signer returned an invalid NIP-17 seal");
    if (!api().verifyEvent(event))
      throw new Error("signer returned an invalid seal signature");
    return event;
  }
  ...
  function wrap(sealEvent, recipient, now) {
    ...
    const event = signer.finalizeEvent(
      { kind: 1059, tags: [["p", recipient]], content, created_at: past(now) },
      secret,
    );
    if (!signer.verifyEvent(event))
      throw new Error("failed to verify gift wrap");
  ...
  async function build(input, options = {}) {
    ...
    if (
      ...
      throw new Error("a NIP-44 signer is required");
  ...
  async function share(input, options = {}) {
    const signer = options.signer || root.tinySigner || root.nostr;
    const query =
      options.queryRelayList || ((pubkey) => queryRelayList(pubkey, options));
    const publish =
      options.publish ||
      ((event, relays) => publishEvent(event, relays, options));
    if (typeof query !== "function" || typeof publish !== "function")
      throw new Error("private NIP-17 delivery is unavailable");
    const sender = await publicKey(signer, options.getPublicKey);
    const recipient = normalizePubkey(input.recipient);
    const timeout = options.timeoutMs;
    const receiverList = relayList(
      await bounded(query(recipient), timeout, "recipient relay lookup"),
      recipient,
    );
    const senderList = relayList(
      await bounded(query(sender), timeout, "sender relay lookup"),
      sender,
    );
    const built = await build(input, options);
    if (built.rumor.pubkey !== sender)
      throw new Error("signer changed during file sharing; try again");
    const deliveries = [];
    for (const target of built.events) {
      ...
      const delivery = await bounded(
        publish(target.event, relays),
        timeout,
        "NIP-17 delivery",
      );
      if (!sent(delivery))
        throw new Error("NIP-17 delivery failed on every inbox relay");
  ...
  root.tiny = root.tiny || {};
  root.tiny.files = { ...root.tiny.files, messages: apiObject };
})(globalThis);
