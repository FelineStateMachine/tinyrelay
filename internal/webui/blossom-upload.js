...
// https://raw.githubusercontent.com/hzrd149/blossom/multi-part-uploads-2/buds/14.md
...
(() => {
  ...
  const abortError = () => { const error = new Error("upload canceled"); error.name = "AbortError"; return error; };
  const verifyDescriptor = (descriptor, hash, size) => {
    if (!descriptor || typeof descriptor !== "object" || descriptor.sha256 !== hash || descriptor.size !== size) throw new Error("invalid Blossom upload descriptor");
    return descriptor;
  };
  ...
  const uploadTask = async (source, options = {}, controller) => {
    ...
    let parsed;
    try { parsed = new URL(base, globalThis.location?.href || "https://blossom.invalid"); } catch { throw new Error("invalid upload URL"); }
    if (parsed.protocol !== "http:" && parsed.protocol !== "https:") throw new Error("upload URL must use HTTP or HTTPS");
    if (parsed.username || parsed.password || parsed.hash || parsed.search) throw new Error("upload URL must not contain credentials, query, or fragment");
    ...
    const target = new RegExp("/" + hash + "(?:\\.[a-z0-9]{1,8})?/?$").test(path) ? base : base.replace(/\/$/, "") + "/" + hash + (extension ? "." + extension : "");
    const url = target;
    const fetcher = options.fetch || globalThis.fetch;
    if (typeof fetcher !== "function") throw new Error("fetch is unavailable");
    const chunkSize = Number.isSafeInteger(options.chunkSize) && options.chunkSize > 0 ? options.chunkSize : 5 * 1024 * 1024;
    ...
    const progress = sent => options.onProgress?.({sent, total: bytes.byteLength, fraction: bytes.byteLength ? sent / bytes.byteLength : 1});
    const request = async (method, body, headers, authBody = body) => {
      if (controller.signal.aborted) throw abortError();
      const requestController = new AbortController();
      let timer, rejectCanceled;
      const cancel = () => { requestController.abort(); rejectCanceled?.(abortError()); };
        ...
        return await Promise.race([
          (async () => {
            ...
            if (options.authorize && method !== "OPTIONS") {
              const value = await options.authorize(url, method, authBody, {hash, type});
            ...
            const response = await fetcher(url, {method, headers: requestHeaders, body: method === "OPTIONS" ? undefined : body, signal: requestController.signal});
            ...
            return {status: response.status, ok: response.ok, headers: response.headers, json: async () => descriptor};
          })(),
          new Promise((_, reject) => {
            timer = setTimeout(() => { requestController.abort(); reject(Error("upload request timed out")); }, timeoutMs);
          }),
          new Promise((_, reject) => { rejectCanceled = reject; })
        ]);
      } finally {
        clearTimeout(timer);
    ...
    const retry = async (action, offset, length) => {
      ...
      while (true) {
        ...
        try {
          ...
          if (!result.ok && result.status >= 500 && attempt++ < retries) { notify(offset, length, "retrying"); continue; }
          return result;
        } catch (error) {
          if (controller.signal.aborted) throw abortError();
    ...
    if (!supportsPatch) {
      ...
      return {hash, descriptor: verifyDescriptor(await responseBody(response), hash, bytes.byteLength), state};
    }

    const metadata = state.meta;
    if (!metadata || metadata.hash !== hash || metadata.size !== bytes.byteLength || metadata.chunkSize !== chunkSize || metadata.url !== url || metadata.type !== type || Date.now() - (metadata.lastAcceptedAt || 0) >= 60000) {
      state.clear();
      state.meta = {hash, size: bytes.byteLength, chunkSize, url, type, lastAcceptedAt: 0};
    }
    let sent = 0;
    let replayed = false;
    while (true) {
      ...
      if (!replayed && prior?.status === 204 && Date.now() - (state.meta.lastAcceptedAt || 0) < 60000) { sent = end; progress(sent); continue; }
      const result = await retry(() => request("PATCH", chunk, {
        "upload-type": type,
        "upload-length": String(bytes.byteLength),
        "upload-offset": String(offset),
        "content-type": "application/octet-stream"
      }, options.authorizationMode === "final-hash" ? bytes : chunk), offset, length);
      ...
      progress(sent);
      if (result.status !== 204) return {hash, descriptor: verifyDescriptor(await responseBody(result), hash, bytes.byteLength), state};
      if (bytes.byteLength === 0) break;
      }
      if (!replayed) { replayed = true; continue; }
  ...
  globalThis.TinyBlossomUpload = Object.freeze({upload});
})();
