...
func TestRoomPageRendersMessagesOldestFirstWithMarkers(t *testing.T) {
	...
	for _, want := range []string{
		...
		`<room-message id="msg-` + roomHello + `" data-id="` + roomHello + `" data-kind="9" data-pubkey="` + roomAgent + `" data-agent>`,
		...
		`<ul id="members"><li><nostr-key hex="` + roomOwner + `"`, `<li data-agent><nostr-key hex="` + roomAgent + `"`, `<small>member | agent</small>`,
