...
(() => {
  ...
  const codec = () => globalThis.TinyBlossomManifests;
  const hex = value => codec().hex(value);
  const fromHex = value => {
    if (!/^[0-9a-f]{64}$/.test(value || "")) throw Error("Invalid hash or encryption key.");
  ...
  const checkAbort = signal => {
    if (signal?.aborted) throw Object.assign(Error("Upload canceled. Retry while this page stays open."), {name: "AbortError"});
  };
  const validName = name => typeof name === "string" && name.length > 0 && name.isWellFormed() && ![".", ".."].includes(name) && !/[\/\u0000]/.test(name) && new TextEncoder().encode(name).length <= 1024;
  ...
  function selectionTree(files) {
    ...
    for (let index = 0; index < files.length; index++) {
      ...
      if (!Number.isSafeInteger(file.size) || file.size < 0 || file.size > MAX_FILE_BYTES) throw Error("Files must be at most 256 MiB in the browser workspace.");
      total += file.size;
  ...
  async function storeEncrypted(plaintext, type, signal) {
    ...
    const result = await TinyBlossomUpload.upload(encrypted.ciphertext, {
      url, hash: encrypted.hash, type, signal,
      authorize: (target, method, body) => tiny.authorization(target, method, body)
    });
    if (result.descriptor.sha256 !== encrypted.hash || result.descriptor.size !== encrypted.ciphertext.length) throw Error("Relay returned an unexpected blob descriptor.");
    return {hash: encrypted.hash, key: fromHex(encrypted.key)};
  }

  const manifestStore = signal => async ({bytes, node}) => storeEncrypted(bytes, node.t === 2 || node.t === 3 ? directoryType : "application/octet-stream", signal);
  const rootReference = details => {
    const stored = details.manifests.at(-1);
    return {hash: stored.hash, key: stored.key, type: details.root.t, size: details.root.l.reduce((sum, link) => sum + link.s, 0)};
  };
  ...
  async function storeFile(file, signal, progress) {
    ...
    return rootReference(await codec().buildFile(chunks, {store: manifestStore(signal), returnDetails: true}));
  }
  ...
  async function storeDirectory(directory, signal, progress) {
    ...
    for (const [name, file] of directory.files) {
      ...
      entries.push({...stored, name, metadata: {type: file.type || "application/octet-stream"}});
    }
    ...
    return rootReference(await codec().buildDirectory(entries, {store: manifestStore(signal), returnDetails: true}));
  }
  ...
  const shareURL = (reference, name, type) => {
    ...
    url.hash = new URLSearchParams({manifest: reference.hash, key: hex(reference.key), node: String(reference.type), name, type: type || "application/octet-stream"}).toString();
    return url.href;
  };
  ...
  class Workspace extends HTMLElement {
    connectedCallback() {
      ...
      this.querySelector("[data-retry]").addEventListener("click", () => this.run().catch(error => this.say(error.message, true)));
    }
    ...
    async run(form) {
      ...
      try {
        ...
        let sent = 0;
        const progress = size => { sent += size; this.say("Stored " + sent.toLocaleString() + " bytes. Encrypting the remaining content…"); };
  ...
  async function fetchBlob(hash, signal) {
    fromHex(hash);
    const response = await fetch(tiny.localPath("/files/raw?hash=" + hash), {signal});
    if (!response.ok) throw Error("Download failed (" + response.status + ").");
    if (Number(response.headers.get("Content-Length")) > MAX_FETCH_BYTES) throw Error("Linked blob exceeds the browser fetch limit.");
    ...
    try {
      for (;;) {
        ...
        if (done) break;
        size += value.length;
        if (size > MAX_FETCH_BYTES) { await reader.cancel(); throw Error("Linked blob exceeds the browser fetch limit."); }
        parts.push(value);
      }
  ...
  async function plaintext(reference, signal) {
    ...
    return reference.k ? TinyBlossomEncryption.decryptCHK(raw, hex(reference.k), hash) : raw;
  }
  const decrypt = (raw, link) => TinyBlossomEncryption.decryptCHK(raw, hex(link.k), hex(link.h));
  ...
  class Root extends HTMLElement {
    connectedCallback() {
      ...
      if (!params.has("manifest") && this.getAttribute("type") !== directoryType) { this.hidden = true; return; }
      this.hidden = false;
    ...
    async load() {
      ...
      this.controller = new AbortController();
      const params = new URLSearchParams(location.hash.slice(1));
      const reference = {h: fromHex(params.get("manifest") || this.getAttribute("hash"))};
      ...
      if (type === 0) { this.offerDownload(data, params.get("name") || "file", params.get("type")); return; }
      ...
      if (type === 1) {
        ...
        button.type = "button";
        button.addEventListener("click", () => this.download({...reference, t: 1, s: root.l.reduce((sum, link) => sum + link.s, 0), n: params.get("name") || "file", m: {type: params.get("type")}}).catch(error => this.fail(error.message)));
        this.append(button); return;
      }
      const entries = await codec().resolveDirectory(root, (hash) => fetchBlob(hash, this.controller.signal), {decrypt, maxDepth: MAX_DEPTH, maxEntries: MAX_FILES, maxBytes: 64 * 1024 * 1024});
      ...
      for (const link of entries) {
        ...
        if (link.t === 2 || link.t === 3) {
          ...
          const fragment = new URLSearchParams({manifest: hex(link.h), node: String(link.t), name: link.n});
          if (link.k) fragment.set("key", hex(link.k));
          url.hash = fragment.toString(); open.href = url.href;
          item.append(open);
        } else {
    ...
    async download(link) {
      ...
      if (link.t === 0) data = await plaintext(link, this.controller.signal);
        ...
        data = await codec().readFile(manifest, hash => fetchBlob(hash, this.controller.signal), {decrypt, maxDepth: MAX_DEPTH, maxBytes: MAX_FILE_BYTES});
    ...
    offerDownload(data, name, type) {
      ...
      link.href = this.downloadURL; link.download = name;
      this.append(link);
      link.click();
    }
  }
