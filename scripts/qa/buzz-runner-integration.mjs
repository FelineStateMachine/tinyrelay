...
try {
  ...
  await publish('owner', 30392, '', [['d', pub.agent], ['p', pub.agent], ['name', 'QA agent'], ['room', 'work'], ['room', 'cancel'], ['room', 'crash'], ['k', '0'], ['k', '9'], ['jobs', 'both'], ['k', '7'], ['k', '1111'], ['k', '20001'], ['jobs', 'both'], ['expiration', String(Math.floor(Date.now() / 1000) + 3600)]]);
  ...
  const request = await waitForEvent({kinds: [43001], '#h': ['work'], '#p': [pub.agent]});
  await waitForEvent({kinds: [43003], '#h': ['work'], '#e': [request.id]});
  ...
  const result = await waitForEvent({kinds: [43004], '#h': ['work'], '#e': [request.id]});
  ...
  const cancelRequest = await waitForEvent({kinds: [43001], '#h': ['cancel'], '#p': [pub.agent]}, event => event.content.includes('Start a task'));
  ...
  const canceled = await waitForEvent({kinds: [43006], '#h': ['cancel'], '#e': [cancelRequest.id]});
  ...
  const crashedRequest = await waitForEvent({kinds: [43001], '#h': ['crash'], '#p': [pub.agent]}, event => event.content.includes('Crash once'));
  const crashed = await waitForEvent({kinds: [43006], '#h': ['crash'], '#e': [crashedRequest.id]});
  ...
  const retryRequest = await waitForEvent({kinds: [43001], '#h': ['crash'], '#p': [pub.agent]}, event => event.content.includes('Recover after crash'));
  ...
  const retryResult = await waitForEvent({kinds: [43004], '#h': ['crash'], '#e': [retryRequest.id]});
