...
(() => {
  ...
  // Feature modules use this narrow contract instead of reaching into the
  // generic component implementation.
  tiny.ui = Object.freeze({FormElement, el, isHex64});
  ...
  class PublishList extends FormElement {
    async submit(form) {
      ...
      const event = await tiny.signing.signEvent({
  ...
  class NostrCompose extends FormElement {
    ...
    async submit(form) {
      ...
      const event = await tiny.signing.publish(unsigned);
      this.report("Published.");
      form.reset();
  ...
  class ProfileForm extends FormElement {
    ...
    async submit(form) {
      ...
      this.report("Signing…");
      const event = await tiny.signing.signEvent(unsigned);
  ...
  const publishSigned = async unsigned => {
    return tiny.signing.publish(unsigned);
  };
  ...
  class RelayLists extends HTMLElement {
    ...
    async add(row) {
      ...
      const event = await tiny.signing.signEvent({kind: Number(row.dataset.kind), created_at: Math.floor(Date.now() / 1000), content: existing?.content || "", tags});
  ...
  (() => {
    ...
    const allowedRoute = path => /^(?:\/(?:inbox|approvals|profile|account|outbox|search|social|articles|private|chat|media|sites|marmot|grasp|terms|signin|connect|tools|repo|repos|file|files|wiki|rooms)?\/?|\/manage(?:\/(?:people|agents|moderation|rules|identity|connect|data|sync|views|health|owner|status))?\/?|\/manage\/agents\/[^/]+\/?|\/(?:invite|e|a|wiki|social)\/.+|\/repos\/[^/]+\/[^/]+(?:\/.*)?|\/file\/[0-9a-f]{64}\/?|\/files\/(?:sites|rooms|storage)\/?|\/approvals\/[0-9a-f]{64}\/?|\/chat\/dm\/[0-9a-f]{64}|\/rooms\/[a-z0-9_-]{1,64}(?:\/thread\/[0-9a-f]{64})?\/?)$/.test(path || "");
