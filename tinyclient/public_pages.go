...
func (a *App) publicFeed(ctx context.Context, tab, actor string, values url.Values) ([]any, error) {
	...
	case "sites":
		filter = map[string]any{"kinds": []int{kindSite, kindNamedSite}, "limit": 100}
	default:
		return nil, nil
	}
...
func (a *App) siteRows(feed []any) []any {
	...
	for _, item := range feed {
		...
		rows = append(rows, map[string]any{"id": e.ID, "kind": e.Kind, "author": e.PubKey, "label": label, "paths": sitePathCount(e), "created_at": e.CreatedAt, "url": scheme + "://" + label + "." + base.Host})
	}
	return rows
}
